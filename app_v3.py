...
if page == "📊 Base Centrale":
    ...
    if 'DATE' not in df_central.columns or df_central['DATE'].isna().all():
        ...
        if len(periodes_brutes) == 0:
            ...
            # ✨ NOUVEAU : Créer une colonne PERIODE_ANALYSE
            # Pour les E1 (DATE_COMPLEMENTAIRE remplie) : utiliser DATE_COMPLEMENTAIRE
            # Pour les autres (E0, E5, BT) : utiliser DATE
            if 'DATE_COMPLEMENTAIRE' in df_filtered.columns:
                df_filtered['PERIODE_ANALYSE'] = df_filtered.apply(
                    lambda row: row['DATE_COMPLEMENTAIRE'] if pd.notna(row['DATE_COMPLEMENTAIRE']) and row['DATE_COMPLEMENTAIRE'] != '' else row['DATE'],
                    axis=1
                )
            else:
                df_filtered['PERIODE_ANALYSE'] = df_filtered['DATE']
            
            # Grouper par PERIODE_ANALYSE au lieu de DATE
            df_grouped = df_filtered.groupby('PERIODE_ANALYSE').agg({
                'MONTANT': 'sum',
                'CONSO': 'sum'
            }).reset_index()
            
            df_grouped = df_grouped.rename(columns={'PERIODE_ANALYSE': 'DATE'})
            ...
            if "Haute Tension" in type_graphique or "Global" in type_graphique:
                ...
                if 'PSABON' in df_ht_puiss.columns and 'PSATTEINTE' in df_ht_puiss.columns:
                    ...
                    # ✨ NOUVEAU : Créer PERIODE_ANALYSE pour les puissances aussi
                    if 'DATE_COMPLEMENTAIRE' in df_ht_puiss.columns:
                        df_ht_puiss['PERIODE_ANALYSE'] = df_ht_puiss.apply(
                            lambda row: row['DATE_COMPLEMENTAIRE'] if pd.notna(row['DATE_COMPLEMENTAIRE']) and row['DATE_COMPLEMENTAIRE'] != '' else row['DATE'],
                            axis=1
                        )
                    else:
                        df_ht_puiss['PERIODE_ANALYSE'] = df_ht_puiss['DATE']
                    
                    if len(df_ht_puiss) > 0:
                        if site_filter == 'Tous':
                            # Vue globale : grouper par PERIODE_ANALYSE
                            df_puiss_grouped = df_ht_puiss.groupby('PERIODE_ANALYSE').agg({
                                'PSABON': 'mean',
                                'PSATTEINTE': 'mean'
                            }).reset_index()
                            
                            df_puiss_grouped = df_puiss_grouped.rename(columns={'PERIODE_ANALYSE': 'DATE'})
                            ...
                            # Vue par site : grouper par PERIODE_ANALYSE pour ce site
                            df_site_puiss = df_ht_puiss.groupby('PERIODE_ANALYSE').agg({
                                'PSABON': 'first',
                                'PSATTEINTE': 'first'
                            }).reset_index()
                            
                            df_site_puiss = df_site_puiss.rename(columns={'PERIODE_ANALYSE': 'DATE'})
