...
def ajouter_lignes_base_centrale(df_base, nouvelles_lignes, periode):
    ...
    # ========================================
    # CORRECTION : Supprimer VRAIS doublons uniquement
    # ✨ IMPORTANT : Ne PAS supprimer les E0 + E1 pour même IDENTIFIANT/DATE
    # ========================================
    
    # Convertir MONTANT en numérique
    df_resultat['MONTANT'] = pd.to_numeric(df_resultat['MONTANT'], errors='coerce').fillna(0)
    
    # ✨ NOUVELLE STRATÉGIE : Créer une clé UNIQUE qui inclut DATE_COMPLEMENTAIRE
    # Cela permet d'avoir E0 + E1 + E5 pour même IDENTIFIANT/DATE
    
    # Normaliser DATE_COMPLEMENTAIRE (vide → 'VIDE' pour le dédoublonnage)
    df_resultat['_DATE_COMP_NORM'] = df_resultat['DATE_COMPLEMENTAIRE'].apply(
        lambda x: str(x) if pd.notna(x) and str(x) != '' and str(x) != 'nan' else 'VIDE'
    )
    
    # Créer clé type basée sur montant
    df_resultat['_TYPE'] = df_resultat['MONTANT'].apply(lambda x: 'NEG' if x < 0 else 'POS')
    
    # ✨ CLÉ COMPLÈTE : IDENTIFIANT + DATE + TYPE + DATE_COMPLEMENTAIRE
    # Ceci permet :
    # - E0 : [ID, DATE, POS, VIDE]
    # - E1 : [ID, DATE, POS, 01/2025]
    # - E5 : [ID, DATE, NEG, VIDE]
    # → Toutes différentes, aucun doublon !
    
    df_resultat = df_resultat.drop_duplicates(
        subset=['IDENTIFIANT', 'DATE', '_TYPE', '_DATE_COMP_NORM'], 
        keep='last'
    )
    
    # Supprimer colonnes temporaires
    df_resultat = df_resultat.drop(columns=['_DATE_COMP_NORM', '_TYPE'])
...
def generer_piece_comptable(df_base, periode, tension=None):
    ...
    # ✨ NOUVELLE APPROCHE : Créer la pièce ligne par ligne (plus sûr)
    lignes_piece = []
    
    for idx, row in df_filtre.iterrows():
        # Récupérer COMPTE_CHARGE
        compte_charge = '62183464'
        if 'COMPTE_CHARGE' in row.index and pd.notna(row['COMPTE_CHARGE']):
            compte_charge = str(row['COMPTE_CHARGE'])
        ...
        if pd.notna(row.get('DATE_COMPLEMENTAIRE')) and str(row.get('DATE_COMPLEMENTAIRE')) != '' and str(row.get('DATE_COMPLEMENTAIRE')) != 'nan':
            libelle = f"{tension_prefix} {date_str} {site} COMPLEMENTAIRE {row['DATE_COMPLEMENTAIRE']}"
        else:
            libelle = f"{tension_prefix} {date_str} {site}"
        
        # Créer la ligne de pièce comptable
        ligne_piece = {
            'CODE AGENCE': row['CODE AGCE'],
            'COMPTE DE CHARGES': compte_charge,
            'SENS': 'D',
            'MONTANT': pd.to_numeric(row['MONTANT'], errors='coerce') or 0,
            'CODE PAYT Lib 1-5': '4200',
            'CODE CHARGE Lib 6-10': '',
            'TYPE DEP Lib 11': '',
            'MATR OBJ Lib 12-19': '',
        ...
        lignes_piece.append(ligne_piece)
    
    # Créer le DataFrame final
    piece = pd.DataFrame(lignes_piece)
