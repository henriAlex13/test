...
def parse_sat(contents, filename, mois):
    ...
    try:
        ...
        for sheet_name in xl.sheet_names:
            ...
            for i in range(hrow + 1, len(df)):
                ...
                # Déterminer si UC ou agence via le mapping statique
    ...
    try:
        ...
        return True, f"{len(rows)} lignes SAT — {nb_uc} UCs, {nb_ag} agences"
    except Exception as e:
        return False, f"Erreur SQLite : {str(e)}"
...
def parse_sat_pro(contents, filename, mois):
    ...
    try:
        ...
        nb_uc = sum(1 for r in rows if r[5] == 1)
        nb_ag = sum(1 for r in rows if r[5] == 0)
        return True, f"{len(rows)} lignes SAT PRO — {nb_uc} UCs, {nb_ag} agences"
    except Exception as e:
        return False, f"Erreur SQLite : {str(e)}"
...
def render_sat(mois_filter, uc_filter=None, agence_filter=None):
    try:
        ...
        for feuille in feuilles:
            ...
            # Graphique sur les agences seulement (pas les lignes UC)
            df_graph = dff.copy()
            if has_is_uc:
                df_graph = df_graph[df_graph["is_uc"] != 1]
            df_graph = df_graph[df_graph["mois"] == last].copy()
            df_graph["valeur_num"] = pd.to_numeric(
                df_graph["valeur"].astype(str).str.replace('%','',regex=False), errors='coerce')
            df_graph = df_graph.dropna(subset=["valeur_num"])

            if not df_graph.empty:
                if has_uc and not agences_filtre and "uc" in df_graph.columns:
                    df_graph["uc"] = df_graph["uc"].fillna("Sans UC")
                    df_graph = df_graph.sort_values(["uc","valeur_num"], ascending=[True,False])
                    fig = px.bar(df_graph, x="entite", y="valeur_num", text="valeur",
                                 color="uc", title=f"{feuille} — {last}",
                                 template="plotly_dark",
                                 color_discrete_sequence=["#58a6ff","#3fb950","#ffa657","#f78166",
                                                           "#d2a8ff","#79c0ff","#56d364","#ffa198",
                                                           "#e3b341","#ff7b72","#a5d6ff","#7ee787"])
                    fig.update_layout(legend_title="UC")
                else:
                    df_graph = df_graph.sort_values("valeur_num", ascending=False)
                    fig = px.bar(df_graph, x="entite", y="valeur_num", text="valeur",
                ...
                content_tab.append(dbc.Row([_gcard(fig, 12)]))
...
def render_sat_pro(mois_filter, uc_filter=None, agence_filter=None):
    ...
    if not tabs:
        return empty_state()
    return dbc.Tabs(tabs, active_tab=f"satpro-{feuilles[0]}")
