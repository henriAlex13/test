...
CONFIG_HT = {
    ...
    'date_suppl_col': 'date_suppl'  # ✨ NOUVEAU : Colonne date complémentaire (future)
}
...
def importer_factures_ht(df_ht, df_base_centrale, periode, has_typefact):
    ...
    for _, row_facture in df_traite.iterrows():
        ...
        # ✨ NOUVEAU : is_e1 peut maintenant être True si date_suppl existe
        ...
        if is_e1 and CONFIG_HT['date_suppl_col'] in row_facture.index:
        ...
        if not ligne_existante.empty:
            ...
            # ✨ NOUVEAU : Utiliser date_complementaire_value (depuis date_suppl) si E1
