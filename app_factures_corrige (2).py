...
def export_base_centrale(df):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Styles d'alignement pré-définis
        ...
        for row_idx, row_data in enumerate(df.itertuples(index=False), start=row_header + 1):
            for col_idx, value in enumerate(row_data, start=1):
                ...
                # Choisir l'alignement selon le type de colonne
                col_name = df.columns[col_idx - 1]
                if col_name in ['MONTANT', 'CONSO']:
                    cell.alignment = align_right
                elif col_name in ['SITES', 'CORRESPONDANCE']:
                    cell.alignment = align_left_wrap
                else:
                    cell.alignment = align_left
                
                # Formater les montants
                if col_name == 'MONTANT' and value:
        ...
        col_widths = {
            'UC': 12,
            'CODE AGCE': 15,
            'SITES': 35,               # Augmenté pour afficher les noms complets
            'CORRESPONDANCE': 30,      # Augmenté
            'IDENTIFIANT': 20,         # Augmenté
            'REFERENCE': 20,           # Augmenté
            'TENSION': 12,
            'MONTANT': 15,
            'CONSO': 15,               # Augmenté
            'DATE': 15
        ...
        for col_idx, col_name in enumerate(df.columns, start=1):
            ...
            # Utiliser la largeur personnalisée si disponible
            if col_name in col_widths:
                width = col_widths[col_name]
            else:
                # Sinon, calculer la largeur basée sur le contenu
                max_length = len(str(col_name))
                for value in df[col_name].astype(str):
                    max_length = max(max_length, len(value))
                # Ajouter un peu de marge et limiter à 50
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for row_idx, row_data in enumerate(df.itertuples(index=False), start=row_header + 1):
            for col_idx, value in enumerate(row_data, start=1):
                ...
                # Choisir l'alignement selon le type de colonne
                col_name = df.columns[col_idx - 1]
                if col_name in ['MONTANT', 'DEBIT', 'CREDIT', 'CONSO']:
                    cell.alignment = align_right
                elif col_name in ['LIBELLE COMPLEMENTAIRE', 'LIBELLE', 'SITES', 'CORRESPONDANCE']:
                    cell.alignment = align_left_wrap
                else:
                    cell.alignment = align_left
                
                # Formater les montants
                if col_name in ['MONTANT', 'DEBIT', 'CREDIT'] and value:
                    cell.number_format = '#,##0'
        ...
        # SECTION RÉCAPITULATIVE (Date et Montant)
        ligne_recap = ligne_vide + 1
        ...
        col_date_label = 1
        col_date_value = 2
        col_montant_label = max(4, nb_cols // 2)
        col_montant_value = col_montant_label + 1
        
        # Date d'édition
        worksheet.cell(row=ligne_recap, column=col_date_label, value="Date d'édition:")
        worksheet.cell(row=ligne_recap, column=col_date_value, value=current_date)
        
        # Montant total
        worksheet.cell(row=ligne_recap, column=col_montant_label, value="MONTANT TOTAL:")
        worksheet.cell(row=ligne_recap, column=col_montant_value, value=f"{montant_total:,.0f} FCFA")
        
        # Appliquer le style
        for col in [col_date_label, col_date_value, col_montant_label, col_montant_value]:
        ...
        # Ligne vide après récap
        ligne_vide_apres_recap = ligne_recap + 2
        ligne_titre_sig = ligne_vide_apres_recap + 1
        ...
        # Définir des largeurs personnalisées selon les colonnes
        col_widths = {
            'UC': 12,
            'CODE AGCE': 18,
            'SITES': 40,                      # Augmenté pour noms complets
            'CORRESPONDANCE': 35,             # Augmenté
            'IDENTIFIANT': 22,                # Augmenté
            'REFERENCE': 22,                  # Augmenté
            'TENSION': 12,
            'MONTANT': 18,                    # Augmenté
            'CONSO': 15,                      # Augmenté
            'DATE': 18,                       # Augmenté
            'LIBELLE COMPLEMENTAIRE': 50,     # Augmenté pour affichage complet
            'N° COMPTE': 18,                  # Augmenté
            'COMPTE': 18,                     # Augmenté
            'N°ORDRE': 12,
            'CODE JOURNAL': 18,               # Augmenté
            'CODE': 15,                       # Augmenté
            'LIBELLE': 45,                    # Augmenté
            'DEBIT': 18,                      # Augmenté
            'CREDIT': 18,                     # Augmenté
        ...
        for col_idx, col_name in enumerate(df.columns, start=1):
            ...
            if col_name in col_widths:
                ...
                # Sinon, calculer la largeur basée sur le contenu
                # Longueur maximale entre le nom de colonne et les valeurs
                max_length = len(str(col_name))
                for value in df[col_name].astype(str):
                    max_length = max(max_length, len(value))
                # Ajouter un peu de marge et limiter à 50
...
with st.sidebar:
    ...
    page = st.radio(
        "Menu principal",
        ["📊 Base Centrale", "🔄 Import Factures BT", "🔄 Import Factures HT", "📈 Statistiques", "⚙️ Génération Fichiers", "🔧 Gestion Manuelle Pièces"]
    )
...
if page == "📊 Base Centrale":
    ...
    if fichier_bt:
        try:
            ...
            typefact_col = "Type de facture"  # NOUVELLE COLONNE
            ...
            # Vérifier si typefact existe (optionnel)
            has_typefact = typefact_col in df_bt.columns
            if has_typefact:
                st.info(f"✅ Colonne '{typefact_col}' détectée - Gestion des factures multiples activée")
            else:
                st.warning(f"⚠️ Colonne '{typefact_col}' non trouvée - Les doublons seront ignorés")
                    ...
                    if st.button("🔄 LANCER L'IMPORT BT", type="primary", use_container_width=True):
                        with st.spinner("⏳ Import BT en cours..."):
                            ...
                            if df_template_bt is None:
                                ...
                                # NOUVELLE LOGIQUE : Grouper par IDENTIFIANT + DATE si typefact = E0
                                if has_typefact:
                                    # Filtrer les lignes E0
                                    df_bt_e0 = df_bt[df_bt[typefact_col] == 'E0'].copy()
                                    df_bt_autres = df_bt[df_bt[typefact_col] != 'E0'].copy()
                                    
                                    # Grouper et cumuler les E0 par IDENTIFIANT
                                    if len(df_bt_e0) > 0:
                                        df_bt_e0_cumul = df_bt_e0.groupby(cle_facture).agg({
                                            montant_col: 'sum',
                                            conso_col: 'sum' if conso_col in df_bt_e0.columns else 'first',
                                            caract_col: 'first'
                                        }).reset_index()
                                        
                                        st.info(f"📊 {len(df_bt_e0)} ligne(s) E0 détectées → {len(df_bt_e0_cumul)} ligne(s) après cumul")
                                    else:
                                        df_bt_e0_cumul = pd.DataFrame()
                                    
                                    # Combiner les E0 cumulées avec les autres
                                    if len(df_bt_autres) > 0:
                                        df_bt_traite = pd.concat([df_bt_e0_cumul, df_bt_autres], ignore_index=True)
                                    else:
                                        df_bt_traite = df_bt_e0_cumul
                                else:
                                    # Pas de colonne typefact, traiter normalement
                                    df_bt_traite = df_bt.copy()
                                
                                nouvelles_lignes = []
                                nb_ajouts = 0
                                
                                for _, row_facture in df_bt_traite.iterrows():
                                    ...
                                    if not ligne_centrale.empty:
                                        ...
                                        nouvelle_ligne = pd.Series({
                                            ...
                                            'CONSO': row_facture.get(conso_col, None) if conso_col in df_bt_traite.columns else None,
                                            'DATE': periode_bt
                                        })
                            ...
                            if nouvelles_lignes:
                                ...
                                if has_typefact and len(df_bt_e0) > 0:
                                    st.info(f"ℹ️ {len(df_bt_e0)} facture(s) E0 cumulées en {len(df_bt_e0_cumul)} ligne(s)")
...
elif page == "🔧 Gestion Manuelle Pièces":
    st.markdown("## 🔧 Gestion Manuelle des Pièces Comptables")
    st.markdown("---")
    ...
    <div style='background: linear-gradient(135deg, #f093fb, #f5576c); 
                ...
                border-radius: 10px;
                margin: 1rem 0;'>
        <h3 style='margin: 0;'>🔧 GESTION MANUELLE</h3>
        <p style='margin: 0.5rem 0 0 0;'>Vérifier et ajuster manuellement les pièces comptables</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Tabs pour BT et HT
    tab_bt_manual, tab_ht_manual = st.tabs(["🔌 Basse Tension (BT)", "⚡ Haute Tension (HT)"])
    
    with tab_bt_manual:
        st.markdown("### 🔌 Gestion Manuelle BT")
        ...
        if df_template_bt is None:
            ...
            if 'DATE' in df_central.columns:
                ...
                if len(periodes_disponibles) > 0:
                    ...
                    periode_selectionnee_bt = st.selectbox(
                        "📅 Sélectionner la période à vérifier",
                        periodes_disponibles,
                        key="periode_manual_bt"
                    )
                    
                    if periode_selectionnee_bt:
                        st.info(f"📊 Période sélectionnée : **{periode_selectionnee_bt}**")
                        
                        # Filtrer la base centrale par période
                        df_central_filtre_bt = df_central[df_central['DATE'] == periode_selectionnee_bt]
                        
                        # Créer un dictionnaire des IDENTIFIANT dans la base centrale
                        identifiants_base = set(df_central_filtre_bt['IDENTIFIANT'].tolist())
                        identifiants_template = set(df_template_bt['IDENTIFIANT'].tolist())
                        
                        # Trouver les lignes NON récupérées (dans template mais pas dans base filtrée)
                        identifiants_non_recuperes = identifiants_template - identifiants_base
                        ...
                        with col_stat1:
                            st.metric("📋 Lignes template", len(df_template_bt))
                        with col_stat2:
                            st.metric("✅ Récupérées", len(identifiants_base))
                        with col_stat3:
                            st.metric("❌ Non récupérées", len(identifiants_non_recuperes))
                        with col_stat4:
                            taux = (len(identifiants_base) / len(identifiants_template) * 100) if len(identifiants_template) > 0 else 0
                            st.metric("📊 Taux", f"{taux:.1f}%")
                        
                        st.markdown("---")
                        
                        if len(identifiants_non_recuperes) > 0:
                            st.warning(f"⚠️ **{len(identifiants_non_recuperes)} ligne(s) non récupérée(s) dans les factures numériques**")
                            
                            # Afficher les lignes non récupérées
                            df_non_recuperes = df_template_bt[df_template_bt['IDENTIFIANT'].isin(identifiants_non_recuperes)].copy()
                            
                            st.markdown("### 📋 Lignes NON récupérées")
                            st.info("💡 Ces lignes du template n'ont pas de correspondance dans les factures numériques importées.")
                            
                            # Afficher avec possibilité de modification
                            edited_non_recuperes = st.data_editor(
                                df_non_recuperes,
                                use_container_width=True,
                                num_rows="dynamic",
                                height=400,
                                key="editor_non_recuperes_bt"
                            )
                            
                            st.markdown("---")
                            
                            # Actions sur les lignes non récupérées
                            col_action1, col_action2, col_action3 = st.columns(3)
                            
                            with col_action1:
                                if st.button("➕ Ajouter à la base centrale", type="primary", use_container_width=True, key="add_non_recup_bt"):
                                    # Ajouter les lignes modifiées à la base centrale avec la DATE
                                    nouvelles_lignes = []
                                    
                                    for _, row in edited_non_recuperes.iterrows():
                                        nouvelle_ligne = pd.Series({
                                            'UC': row.get('UC', ''),
                                            'CODE AGCE': row.get('CODE AGCE', ''),
                                            'SITES': row.get('SITES', ''),
                                            'CORRESPONDANCE': row.get('CORRESPONDANCE', ''),
                                            'IDENTIFIANT': row['IDENTIFIANT'],
                                            'REFERENCE': row.get('REFERENCE', ''),
                                            'TENSION': 'BASSE',
                                            'MONTANT': row.get('MONTANT', 0),
                                            'CONSO': row.get('CONSO', 0),
                                            'DATE': periode_selectionnee_bt
                                        })
                                        nouvelles_lignes.append(nouvelle_ligne)
                                    
                                    if nouvelles_lignes:
                                        df_nouvelles = pd.DataFrame(nouvelles_lignes)
                                        st.session_state.df_central = pd.concat([st.session_state.df_central, df_nouvelles], ignore_index=True)
                                        
                                        # Supprimer les doublons
                                        st.session_state.df_central = st.session_state.df_central.drop_duplicates(subset=['IDENTIFIANT', 'DATE'], keep='last')
                                        
                                        save_central(st.session_state.df_central)
                                        st.success(f"✅ {len(nouvelles_lignes)} ligne(s) ajoutée(s) à la base centrale !")
                                        st.rerun()
                            
                            with col_action2:
                                if st.button("💾 Sauvegarder dans template", use_container_width=True, key="save_template_bt"):
                                    # Mettre à jour le template avec les modifications
                                    for idx, row in edited_non_recuperes.iterrows():
                                        if idx in df_template_bt.index:
                                            df_template_bt.loc[idx] = row
                                    
                                    df_template_bt.to_excel(FICHIER_TEMPLATE_BT, index=False)
                                    st.success("✅ Template BT mis à jour !")
                            
                            with col_action3:
                                if st.button("🔄 Actualiser", use_container_width=True, key="refresh_manual_bt"):
                                    st.rerun()
                        
                        else:
                            st.success("✅ **Toutes les lignes du template ont été récupérées !**")
                        
                        st.markdown("---")
                        
                        # Section : Éditer la pièce comptable complète
                        st.markdown("### 📝 Éditer la Pièce Comptable Complète")
                        st.info("💡 Visualisez et modifiez TOUTES les lignes de la pièce comptable (récupérées + non récupérées)")
                        
                        # Créer la pièce complète en combinant base centrale et template
                        df_piece_complete = df_template_bt.copy()
                        
                        # Mettre à jour avec les données de la base centrale filtrée
                        for idx, row_template in df_piece_complete.iterrows():
                            identifiant = row_template['IDENTIFIANT']
                            ligne_base = df_central_filtre_bt[df_central_filtre_bt['IDENTIFIANT'] == identifiant]
                            
                            if not ligne_base.empty:
                                if 'MONTANT' in ligne_base.columns:
                                    df_piece_complete.at[idx, 'MONTANT'] = ligne_base.iloc[0]['MONTANT']
                                if 'SITES' in ligne_base.columns:
                                    df_piece_complete.at[idx, 'SITES'] = ligne_base.iloc[0]['SITES']
                                if 'LIBELLE COMPLEMENTAIRE' in df_piece_complete.columns:
                                    site = ligne_base.iloc[0]['SITES'] if 'SITES' in ligne_base.columns else ''
                                    df_piece_complete.at[idx, 'LIBELLE COMPLEMENTAIRE'] = f"CIE BT {periode_selectionnee_bt} {site}"
                        
                        # Éditeur complet
                        edited_piece_complete = st.data_editor(
                            df_piece_complete,
                            use_container_width=True,
                            num_rows="dynamic",
                            height=500,
                            key="editor_piece_complete_bt"
                        )
                        
                        st.markdown("---")
                        
                        # Actions sur la pièce complète
                        col_final1, col_final2, col_final3 = st.columns(3)
                        
                        with col_final1:
                            if st.button("💾 Sauvegarder template", use_container_width=True, key="save_final_bt"):
                                edited_piece_complete.to_excel(FICHIER_TEMPLATE_BT, index=False)
                                st.success("✅ Template BT sauvegardé !")
                        
                        with col_final2:
                            # Export avec signatures
                            df_export_final = edited_piece_complete.drop(columns=['IDENTIFIANT']) if 'IDENTIFIANT' in edited_piece_complete.columns else edited_piece_complete
                            excel_data_final = export_factures_cie(df_export_final, type_tension="BT")
                            
                            st.download_button(
                                "📥 Télécharger Excel BT",
                                data=excel_data_final,
                                file_name=f"FACTURAT_BT_MANUEL_{periode_selectionnee_bt}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True,
                                key="dl_final_bt"
                            )
                        ...
                        with col_final3:
                            if st.button("🔄 Actualiser", use_container_width=True, key="refresh_final_bt"):
                                st.rerun()
    ...
    with tab_ht_manual:
        st.markdown("### ⚡ Gestion Manuelle HT")
        ...
        if df_template_ht is None:
            ...
            if 'DATE' in df_central.columns:
                ...
                if len(periodes_disponibles) > 0:
                    # Sélection de période
                    periode_selectionnee_ht = st.selectbox(
                        "📅 Sélectionner la période à vérifier",
                        periodes_disponibles,
                        key="periode_manual_ht"
                    )
                    
                    if periode_selectionnee_ht:
                        st.info(f"📊 Période sélectionnée : **{periode_selectionnee_ht}**")
                        
                        # Filtrer la base centrale par période
                        df_central_filtre_ht = df_central[df_central['DATE'] == periode_selectionnee_ht]
                        
                        # Créer un dictionnaire des IDENTIFIANT dans la base centrale
                        identifiants_base = set(df_central_filtre_ht['IDENTIFIANT'].tolist())
                        identifiants_template = set(df_template_ht['IDENTIFIANT'].tolist())
                        
                        # Trouver les lignes NON récupérées
                        identifiants_non_recuperes = identifiants_template - identifiants_base
                        ...
                        with col_stat1:
                            st.metric("📋 Lignes template", len(df_template_ht))
                        with col_stat2:
                            st.metric("✅ Récupérées", len(identifiants_base))
                        with col_stat3:
                            st.metric("❌ Non récupérées", len(identifiants_non_recuperes))
                        ...
                        if len(identifiants_non_recuperes) > 0:
                            ...
                            # Afficher les lignes non récupérées
                            df_non_recuperes = df_template_ht[df_template_ht['IDENTIFIANT'].isin(identifiants_non_recuperes)].copy()
                            ...
                            with col_action1:
                                if st.button("➕ Ajouter à la base centrale", type="primary", use_container_width=True, key="add_non_recup_ht"):
                                    # Ajouter les lignes modifiées à la base centrale avec la DATE
                                    nouvelles_lignes = []
                            ...
                            with col_action2:
                                if st.button("💾 Sauvegarder dans template", use_container_width=True, key="save_template_ht"):
                                    # Mettre à jour le template avec les modifications
                                    for idx, row in edited_non_recuperes.iterrows():
                                        if idx in df_template_ht.index:
                                            df_template_ht.loc[idx] = row
                                    
                                    df_template_ht.to_excel(FICHIER_TEMPLATE_HT, index=False)
                                    st.success("✅ Template HT mis à jour !")
                            
                            with col_action3:
                                if st.button("🔄 Actualiser", use_container_width=True, key="refresh_manual_ht"):
                                    st.rerun()
                        ...
                        df_piece_complete = df_template_ht.copy()
                        
                        # Mettre à jour avec les données de la base centrale filtrée
                        for idx, row_template in df_piece_complete.iterrows():
                            identifiant = row_template['IDENTIFIANT']
                            ligne_base = df_central_filtre_ht[df_central_filtre_ht['IDENTIFIANT'] == identifiant]
                            ...
                            if not ligne_base.empty:
                                ...
                                if 'LIBELLE COMPLEMENTAIRE' in df_piece_complete.columns:
                                    site = ligne_base.iloc[0]['SITES'] if 'SITES' in ligne_base.columns else ''
                                    df_piece_complete.at[idx, 'LIBELLE COMPLEMENTAIRE'] = f"CIE HT {periode_selectionnee_ht} {site}"
                        ...
                        with col_final1:
                            if st.button("💾 Sauvegarder template", use_container_width=True, key="save_final_ht"):
                                edited_piece_complete.to_excel(FICHIER_TEMPLATE_HT, index=False)
                                st.success("✅ Template HT sauvegardé !")
                        
                        with col_final2:
                            # Export avec signatures
                            df_export_final = edited_piece_complete.drop(columns=['IDENTIFIANT']) if 'IDENTIFIANT' in edited_piece_complete.columns else edited_piece_complete
                            excel_data_final = export_factures_cie(df_export_final, type_tension="HT")
                            
                            st.download_button(
                                "📥 Télécharger Excel HT",
                                data=excel_data_final,
                                file_name=f"FACTURAT_HT_MANUEL_{periode_selectionnee_ht}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True,
                                key="dl_final_ht"
                            )
                        ...
                        with col_final3:
                            if st.button("🔄 Actualiser", use_container_width=True, key="refresh_final_ht"):
                                st.rerun()
