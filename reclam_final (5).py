...
def parse_crc(contents, filename, mois):
    ...
    try:
        ...
        # Ligne 3 = en-tête (0-indexed: ligne 2), col B=objectif, rest=mois
        # Structure: row2=headers, rowN=données
        header_row = None
        for i, row in xf.iterrows():
            vals = [str(v).strip() for v in row if str(v).strip() not in ('nan','')]
            if 'Indicateurs de performance' in vals or 'NOS REALISATIONS' in vals:
                header_row = i
                break
        if header_row is None:
            # fallback: prendre ligne 2
            header_row = 2
        data_start = header_row + 1
        for i in range(data_start, len(xf)):
            indic = str(xf.iloc[i, 1]).strip()
            obj   = str(xf.iloc[i, 2]).strip()
            if indic in ('nan', '') or indic.upper() == 'NAN':
                continue
            # Chercher la colonne correspondant au mois uploadé
            # On prend la dernière colonne non-vide comme valeur courante
            vals_row = xf.iloc[i, 3:].dropna()
    ...
    children=[
        ...
        dbc.Row([
            ...
            dbc.Col([html.Div([
                ...
                dbc.Button([html.I(className="bi bi-trash me-1"), "Vider l'historique"],
                    id="btn-clear-db", color="danger", outline=True, size="sm",
                    className="w-100", style={**btn_style, "fontSize": "11px"}),
                dbc.Modal([
                    dbc.ModalHeader("Confirmation"),
                    dbc.ModalBody("Supprimer tout l'historique KPI ? Cette action est irréversible."),
                    dbc.ModalFooter([
                        dbc.Button("Annuler", id="btn-clear-cancel", color="secondary", size="sm"),
                        dbc.Button("Supprimer", id="btn-clear-confirm", color="danger", size="sm"),
                    ]),
                ], id="modal-clear", is_open=False),
                html.Div(id="clear-status", style={"fontSize": "11px", "marginTop": "8px"}),
            ...
            # ── Zone principale ──
            dbc.Col([html.Div([
                dbc.Tabs(id="kpi-tabs", active_tab="tab-crc", children=[
...
def register_kpi_callbacks(app):
    ...
    def upload_crc(contents, filename, mois, annee):
        ...
        if not mois or not annee:
            return html.Span("⚠ Sélectionne le mois et l'année d'abord", style={"color": "#ffa657"}), []
    ...
    # ── Vider la DB ──────────────────────────────────────
    @app.callback(
        Output("modal-clear", "is_open"),
        Input("btn-clear-db", "n_clicks"),
        Input("btn-clear-cancel", "n_clicks"),
        Input("btn-clear-confirm", "n_clicks"),
        State("modal-clear", "is_open"),
        prevent_initial_call=True,
    )
    def toggle_modal(n1, n2, n3, is_open):
        tid = ctx.triggered_id
        if tid == "btn-clear-db":     return True
        if tid == "btn-clear-cancel": return False
        if tid == "btn-clear-confirm":
            con = sqlite3.connect(DB_PATH)
            con.execute("DELETE FROM kpi_crc")
            con.execute("DELETE FROM kpi_sat")
            con.execute("DELETE FROM kpi_sat_pro")
            con.commit(); con.close()
            return False
        return is_open
    ...
    def clear_status(n):
        return html.Span("✅ Historique vidé", style={"color": "#3fb950"})
