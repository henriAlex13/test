...
def export_piece_comptable_excel(df, type_tension="BT"):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        # Calculer le total (convertir en numérique d'abord)
        ...
        # SECTION SIGNATURES
        # ========================================
        ligne_signatures = ligne_recap + 3
        
        # Titre section
        worksheet.merge_cells(f'A{ligne_signatures}:E{ligne_signatures}')
        cell_titre_sign = worksheet[f'A{ligne_signatures}']
        cell_titre_sign.value = "SIGNATURES ET APPROBATIONS"
        cell_titre_sign.font = Font(bold=True, size=12, name='Calibri', color='FFFFFF')
        cell_titre_sign.fill = PatternFill(start_color=header_color, end_color=header_color, fill_type='solid')
        cell_titre_sign.alignment = Alignment(horizontal='center', vertical='center')
        cell_titre_sign.border = Border(
            left=Side(style='medium'), right=Side(style='medium'),
            top=Side(style='medium'), bottom=Side(style='medium')
        )
        worksheet.row_dimensions[ligne_signatures].height = 25
        
        # Cadres signatures
        ligne_cadres = ligne_signatures + 2
        
        # Cadre 1 : Préparé par
        worksheet.merge_cells(f'A{ligne_cadres}:B{ligne_cadres + 4}')
        cell_prep = worksheet[f'A{ligne_cadres}']
        cell_prep.value = "Préparé par :\n\n\n\nNom : _________________\nDate : _______________"
        cell_prep.font = Font(size=10, name='Calibri')
        cell_prep.alignment = Alignment(horizontal='left', vertical='top', wrap_text=True)
        cell_prep.border = Border(
            left=Side(style='medium'), right=Side(style='medium'),
            top=Side(style='medium'), bottom=Side(style='medium')
        )
        
        # Cadre 2 : Vérifié par
        worksheet.merge_cells(f'C{ligne_cadres}:D{ligne_cadres + 4}')
        cell_verif = worksheet[f'C{ligne_cadres}']
        cell_verif.value = "Vérifié par :\n\n\n\nNom : _________________\nDate : _______________"
        cell_verif.font = Font(size=10, name='Calibri')
        cell_verif.alignment = Alignment(horizontal='left', vertical='top', wrap_text=True)
        cell_verif.border = Border(
            left=Side(style='medium'), right=Side(style='medium'),
            top=Side(style='medium'), bottom=Side(style='medium')
        )
        
        # Cadre 3 : Approuvé par
        worksheet.merge_cells(f'E{ligne_cadres}:F{ligne_cadres + 4}')
        cell_appro = worksheet[f'E{ligne_cadres}']
        cell_appro.value = "Approuvé par :\n\n\n\nNom : _________________\nDate : _______________"
        cell_appro.font = Font(size=10, name='Calibri')
        cell_appro.alignment = Alignment(horizontal='left', vertical='top', wrap_text=True)
        cell_appro.border = Border(
            left=Side(style='medium'), right=Side(style='medium'),
            top=Side(style='medium'), bottom=Side(style='medium')
        )
        
        # Hauteur des cadres
        for row in range(ligne_cadres, ligne_cadres + 5):
            worksheet.row_dimensions[row].height = 20
        
        # Footer
        ligne_footer = ligne_cadres + 6
        worksheet.merge_cells(f'A{ligne_footer}:F{ligne_footer}')
        cell_footer = worksheet[f'A{ligne_footer}']
        cell_footer.value = f"Document généré automatiquement le {current_date} - Société Générale Côte d'Ivoire"
        cell_footer.font = Font(size=9, name='Calibri', italic=True, color='666666')
        cell_footer.alignment = Alignment(horizontal='center', vertical='center')
    ...
    with tab_bt:
        ...
        if len(periodes_disponibles) == 0:
            ...
            if periode_bt:
                ...
                # Générer la pièce
                piece_bt = generer_piece_comptable(df_central, periode_bt, tension='BASSE')
                ...
                if len(piece_bt) == 0:
                    ...
                    # Avertissement sites inactifs
                    if hasattr(piece_bt, 'attrs') and 'nb_sites_inactifs_exclus' in piece_bt.attrs:
                        nb_exclus = piece_bt.attrs['nb_sites_inactifs_exclus']
                        if nb_exclus > 0:
                            st.info(f"ℹ️ {nb_exclus} site(s) INACTIF exclus de cette pièce comptable")
    ...
    with tab_ht:
        ...
        if len(periodes_disponibles) == 0:
            ...
            if periode_ht:
                ...
                # Générer la pièce
                piece_ht = generer_piece_comptable(df_central, periode_ht, tension='HAUTE')
                ...
                if len(piece_ht) == 0:
                    ...
                    # Avertissement sites inactifs
                    if hasattr(piece_ht, 'attrs') and 'nb_sites_inactifs_exclus' in piece_ht.attrs:
                        nb_exclus = piece_ht.attrs['nb_sites_inactifs_exclus']
                        if nb_exclus > 0:
                            st.info(f"ℹ️ {nb_exclus} site(s) INACTIF exclus de cette pièce comptable")
