...
C = {
    ...
    "accent":"#58a6ff","green":"#3fb950","red":"#f78166","orange":"#ffa657",
    "muted":"#8b949e","text":"#e6edf3","border":"#30363d","violet":"#6e40c9",
}
PLOTLY_BASE = dict(
...
card_style={"borderRadius":"12px","border":"1px solid #30363d","backgroundColor":"#1c2128","padding":"16px","marginBottom":"16px"}
btn_style={"borderRadius":"8px","fontWeight":"600","fontSize":"13px","border":"none","cursor":"pointer","letterSpacing":"0.3px"}
label_style={"color":"#8b949e","fontWeight":"500","fontSize":"11px","marginBottom":"5px","display":"block","textTransform":"uppercase","letterSpacing":"0.8px"}
...
def kpi_card(label,value,icon,color,suffix=""):
    return dbc.Col(html.Div([
        html.Div([html.Span(icon,style={"fontSize":"20px","marginRight":"8px"}),
                  html.Span(label,style={"fontSize":"11px","color":"#8b949e","textTransform":"uppercase","letterSpacing":"0.8px","fontWeight":"600"})],
                 style={"display":"flex","alignItems":"center","marginBottom":"8px"}),
        html.Div(f"{value}{suffix}",style={"fontSize":"26px","fontWeight":"800","color":color,"lineHeight":"1"}),
    ],style={"backgroundColor":"#1c2128","border":"1px solid #30363d","borderLeft":f"3px solid {color}","borderRadius":"10px","padding":"14px 18px","marginBottom":"16px"}),width=3)
...
DURATION_PATTERNS={'days':re.compile(r'(\d+)\s*[dDjJ]'),'hours':re.compile(r'(\d+)\s*[hH]'),'minutes':re.compile(r'(\d+)\s*[mM]'),'seconds':re.compile(r'(\d+)\s*[sS]')}
...
def load_data(df):
    ...
    df["SLA Réclamation"]=df["SLA Réclamation"].str.replace('[','',regex=False).str.replace('REC','',regex=False).str.replace('-','',regex=False).str.replace(']','',regex=False)
    df['SLA Réclamation']=df['SLA Réclamation'].str.split(',').explode().reset_index(drop=True)
    ...
    time_columns=["Time Technical Study","Temps Infos complémentaires","Temps Traitemen","Temps SUPPORT","Temps Traitée","Temps A Terminer","Temps Initialisation","Temps Valider Regularisation","Time In the process of Régularisation","Temps Attente retour tiers"]
    ...
    df['DATE_RECLAMATION']=df['Date de résolution'].where(df['Date de résolution'].notna(),df['DATE_AUJOURDUI']).sub(df['Date de création']).dt.days
...
def parse_contents(contents,filename):
    ...
    try:
        if filename.endswith('.csv'): df=pd.read_csv(io.StringIO(decoded.decode('utf-8')),header=4)
        elif filename.endswith('.xlsx') or filename.endswith('.xls'): df=pd.read_excel(io.BytesIO(decoded),engine='openpyxl',header=4)
...
def create_datatable_from_df(df,page_size=10):
    ...
    return dash_table.DataTable(
        ...
        style_cell={'textAlign':'left','minWidth':'120px','whiteSpace':'normal','color':'white','backgroundColor':'#22272e','border':'none'},
...
def generate_view_1(data):
    ...
    fig1=px.pie(nat,values='nombre',names='NATURE',title="Répartition par nature",hole=0.55,color_discrete_sequence=["#58a6ff","#3fb950","#f78166","#ffa657"],template='plotly_dark')
    ...
    fig2=go.Figure(go.Scatter(x=ev['Mois'],y=ev['nombre'],mode='lines+markers+text',text=ev['nombre'],textposition='top center',line=dict(color="#58a6ff",width=2.5),marker=dict(size=8,color="#58a6ff"),fill='tozeroy',fillcolor="rgba(88,166,255,0.08)",hovertemplate="<b>%{x}</b><br>%{y} réclamations<extra></extra>"))
    ...
    ag=data.groupby("AGENCE")["Réf. Réclamation"].count().reset_index(name='nombre').sort_values('nombre').tail(10)
    fig3=px.bar(ag,y="AGENCE",x='nombre',text='nombre',orientation='h',title='Top 10 agences',color='nombre',color_continuous_scale=px.colors.sequential.Blues,template='plotly_dark')
    fig3.update_traces(textfont_size=12,marker_line_width=0,hovertemplate="<b>%{y}</b><br>%{x} réclamations<extra></extra>")
...
def generate_view_2(data):
    graphs=[dbc.Col(kpi_row(data),width=12)]
    gr=data.groupby("GROUPE RESOLUTION")["Réf. Réclamation"].count().reset_index(name='nombre').sort_values('nombre',ascending=False).head(5)
    gr['lbl']=gr['GROUPE RESOLUTION'].apply(lambda x:add_line_breaks(x,max_chars=15))
    fig1=px.bar(gr,x='lbl',y='nombre',text='nombre',title="Top 5 — Groupe de résolution",color='nombre',color_continuous_scale=px.colors.sequential.Purples,template='plotly_dark')
    ...
    graphs.append(dbc.Col(html.Div(dcc.Graph(figure=fig1,config={"displayModeBar":False}),style=card_style),width=6))
    ag=data.groupby("AGENCE")["Réf. Réclamation"].count().reset_index(name='nombre').sort_values('nombre').tail(10)
    fig2=px.bar(ag,y='AGENCE',x='nombre',text='nombre',orientation='h',title='Top 10 agences',color='nombre',color_continuous_scale=px.colors.sequential.Teal,template='plotly_dark')
...
def generate_view_3(data):
    graphs=[dbc.Col(kpi_row(data),width=12)]
    if 'Créateur' in data.columns:
        cr=data.groupby("Créateur")["Réf. Réclamation"].count().reset_index(name='nombre').sort_values('nombre',ascending=False).head(10)
        cr['lbl']=cr['Créateur'].apply(lambda x:add_line_breaks(x,max_chars=15))
        fig1=px.bar(cr,x='lbl',y='nombre',text='nombre',title="Top 10 — Par créateur",color='nombre',color_continuous_scale=px.colors.sequential.Reds,template='plotly_dark')
        fig1.update_traces(textfont_size=12,marker_line_width=0)
        fig1.update_layout(coloraxis_showscale=False,xaxis_tickangle=0)
        apply_layout(fig1,yaxis=dict(visible=False),xaxis=dict(showgrid=False,zeroline=False,color="#8b949e",tickfont=dict(size=11)))
    ...
    fig2=px.bar(ty,x='lbl',y='nombre',text='nombre',title="Répartition par typologie",color='nombre',color_continuous_scale=px.colors.sequential.Sunset,template='plotly_dark')
    fig2.update_traces(textfont_size=12,marker_line_width=0)
...
def generate_view_4(data):
    ...
    fig0=px.pie(dl,values='c',names='d',title="Respect des délais",hole=0.55,color_discrete_map={"PAS HORS DELAI":"#3fb950","HORS DELAI":"#f78166"},template='plotly_dark')
    ...
    fig1=px.bar(cn,x='lbl',y='nombre',text='nombre',title="Canaux de notification",color='nombre',color_continuous_scale=px.colors.sequential.Teal,template='plotly_dark')
    fig1.update_traces(textfont_size=12,marker_line_width=0)
    fig1.update_layout(coloraxis_showscale=False,xaxis_tickangle=0)
    apply_layout(fig1,yaxis=dict(visible=False),xaxis=dict(showgrid=False,zeroline=False,color="#8b949e",tickfont=dict(size=11)))
    graphs.append(dbc.Col(html.Div(dcc.Graph(figure=fig1,config={"displayModeBar":False}),style=card_style),width=6))
...
def generate_sla_ticket_table(data):
    mean_sla=data.pivot_table(index='Réf. Réclamation',values='SLA_JOURS',columns=['SLA_ETAPE'],aggfunc='mean').reset_index()
    mean_sla_rounded=mean_sla.round(2)
    table=[
    ...
    html.Div([
        ...
        html.Div([
            html.Div([
                ...
                dbc.Button("Ouvrir le dashboard",id="btn-open-dashboard",color="primary",style={"borderRadius":"8px","fontWeight":"600","width":"100%"}),
            ],style={"backgroundColor":"#1c2128","border":"1px solid #30363d","borderTop":"3px solid #58a6ff","borderRadius":"12px","padding":"28px","flex":"1","minWidth":"220px","textAlign":"center"}),
            html.Div([
                html.Div("🌐",style={"fontSize":"36px","marginBottom":"12px"}),
                ...
                html.A(dbc.Button("Ouvrir le lien",color="success",style={"borderRadius":"8px","fontWeight":"600","width":"100%"}),
                       href="https://did-spymarketbank-sgci.onrender.com/",target="_blank"),
            ],style={"backgroundColor":"#1c2128","border":"1px solid #30363d","borderTop":"3px solid #3fb950","borderRadius":"12px","padding":"28px","flex":"1","minWidth":"220px","textAlign":"center"}),
            html.Div([
                html.Div("📈",style={"fontSize":"36px","marginBottom":"12px"}),
                html.H4("Tableau de Bord KPI",style={"color":"white","fontWeight":"700","marginBottom":"8px"}),
                html.P("Suivez les KPI : CRC, Satisfaction, SAT PRO.",style={"color":"#8b949e","fontSize":"13px","marginBottom":"20px"}),
                dbc.Button("Ouvrir le module KPI",id="btn-open-module",color="warning",style={"borderRadius":"8px","fontWeight":"600","width":"100%"}),
            ],style={"backgroundColor":"#1c2128","border":"1px solid #30363d","borderTop":"3px solid #ffa657","borderRadius":"12px","padding":"28px","flex":"1","minWidth":"220px","textAlign":"center"}),
        ],style={"display":"flex","gap":"20px","flexWrap":"wrap","justifyContent":"center"}),
    ],style={"maxWidth":"900px","margin":"0 auto","textAlign":"center"}),
],style={"backgroundColor":"#0d1117","minHeight":"100vh","display":"flex","alignItems":"center","justifyContent":"center","padding":"40px 20px"})

# ── DASHBOARD
DD_STYLE={"color":"#000","fontSize":"13px"}
def filter_block(label,comp):
    return html.Div([html.Label(label,style=label_style),comp,html.Div(style={"marginBottom":"14px"})])

dashboard=dbc.Container(fluid=True,style={"backgroundColor":"#0d1117","minHeight":"100vh","padding":"0"},children=[
    html.Div([dbc.Row([
        dbc.Col(html.Img(src=image_path,style={"height":"38px"}),width="auto"),
        dbc.Col(html.Span("Dashboard Réclamations",style={"color":"#e6edf3","fontWeight":"700","fontSize":"17px","lineHeight":"38px"}),width="auto"),
        dbc.Col(dbc.Button("← Accueil",id="btn-retour-accueil",color="secondary",size="sm",style={**btn_style,"fontSize":"12px"}),width="auto",style={"marginLeft":"auto"}),
    ],align="center")],style={"backgroundColor":"#161b22","borderBottom":"1px solid #30363d","padding":"12px 24px","position":"sticky","top":"0","zIndex":"100"}),
    dbc.Row([
        dbc.Col([html.Div([
            html.Div([html.Span("⚙ ",style={"opacity":"0.7"}),html.Span("Filtres",style={"color":"#e6edf3","fontWeight":"700","fontSize":"13px","textTransform":"uppercase","letterSpacing":"1px"})],
            ...
            dbc.Button([html.I(className="bi bi-arrow-counterclockwise me-2"),"Réinitialiser"],id="reset-filters",color="danger",outline=True,size="sm",className="w-100 mt-1",style={**btn_style,"fontSize":"12px"}),
            dcc.Upload(id="upload-data",
                children=html.Div([html.Span("☁",style={"fontSize":"26px","display":"block","marginBottom":"4px"}),html.Span("Glisser / cliquer",style={"fontSize":"12px","color":"#e6edf3","fontWeight":"600"}),html.Br(),html.Span("CSV ou Excel",style={"fontSize":"11px","color":"#8b949e"})],style={"textAlign":"center"}),
                style={"marginTop":"20px","borderWidth":"1.5px","borderStyle":"dashed","borderColor":"#58a6ff","borderRadius":"10px","padding":"18px 8px","cursor":"pointer","backgroundColor":"rgba(88,166,255,0.04)"},
                multiple=False),
        ],style={"backgroundColor":"#161b22","borderRight":"1px solid #30363d","padding":"20px 16px","minHeight":"calc(100vh - 64px)"})],width=2,style={"padding":"0"}),
        dbc.Col([html.Div([
            dbc.Row([
                ...
                dbc.Col(dbc.DropdownMenu(label="📋 SLA PAR",children=items,id="sla-dropdown",color="info",className="w-100"),width=2),
            ],className="mb-4 g-2"),
            html.Div(id="content",style={"minHeight":"400px"}),
            dcc.Store(id="stored-data"),
            dcc.Store(id="stored-active-view"),
        ],style={"padding":"24px"})],width=10,style={"padding":"0"}),
    ],style={"margin":"0"}),
])

# ── MODULE KPI défini plus bas ──────────────────────────────────────────────
...
def parse_crc(contents, filename, mois):
    ...
    try:
        ...
        # Sauter les lignes intermédiaires (labels, sous-titres)
...
def render_sat(mois_filter, uc_filter=None, agence_filter=None):
    f1 = render_feuille1("sat", mois_filter)
    ...
    try:
        ...
        if not tabs:
            return html.Div([f1]) if f1 else empty_state()
        return html.Div([f1, dbc.Tabs(tabs, active_tab=f"sat-{feuilles[0]}")] if f1
                        else [dbc.Tabs(tabs, active_tab=f"sat-{feuilles[0]}")])
...
# ══════════════════════════════════════════════
# LAYOUT RACINE (après définition de module)
# ══════════════════════════════════════════════
# ── LAYOUT RACINE
app.layout=html.Div([
    html.Div(accueil,id="page-accueil",style={"display":"block"}),
    html.Div(dashboard,id="page-dashboard",style={"display":"none"}),
    html.Div(module,id="page-module",style={"display":"none"}),  # rempli après par module
])

# ── CALLBACK NAVIGATION
@app.callback(
    Output("page-accueil","style"),Output("page-dashboard","style"),Output("page-module","style"),
    Input("btn-open-dashboard","n_clicks"),Input("btn-open-module","n_clicks"),
    Input("btn-retour-accueil","n_clicks"),Input("btn-retour-accueil-module","n_clicks"),
    prevent_initial_call=True,
)
def navigate(n1,n2,n3,n4):
    show={"display":"block"};hide={"display":"none"}
    tid=ctx.triggered_id
    if tid=="btn-open-dashboard": return hide,show,hide
    if tid=="btn-open-module": return hide,hide,show
    return show,hide,hide

# ── LECTURE ROBUSTE DU STORE
def read_stored_df(stored_json):
    if stored_json is None: return pd.DataFrame()
    try:
        if isinstance(stored_json,str): return pd.read_json(io.StringIO(stored_json),orient='split')
        ...
        if isinstance(stored_json,list): return pd.DataFrame(stored_json)
    except Exception as e: print(f"[read_stored_df] {e}")
    return pd.DataFrame()

# ── CALLBACKS DASHBOARD
@app.callback(Output('stored-data','data'),Input('upload-data','contents'),State('upload-data','filename'),prevent_initial_call=True)
...
@app.callback(
    ...
    [Input('ticket-filter','value'),Input('agence-filter','value'),
     ...
     Input("btn-1","n_clicks"),Input("btn-2","n_clicks"),Input("btn-3","n_clicks"),Input("btn-4","n_clicks"),
...
def update_all_filters(ticket_val,agence_val,annee_val,mois_val,segmentation_val,delai_val,
    ...
    if trigger_id=="reset-filters": ticket_val=agence_val=annee_val=mois_val=segmentation_val=delai_val=None
    ...
    filters={"Réf. Réclamation":ticket_val,"AGENCE":agence_val,"Annee":annee_val,"Mois":mois_val,"SEGMENTATION":segmentation_val,"DELAI_RECLAMATION":delai_val}

    def filter_df(dframe,filter_dict,exclude_key=None):
        df_f=dframe
        for key,val in filter_dict.items():
            if val is not None and val!=[] and key!=exclude_key and key in df_f.columns:
    ...
    if stored_json is None:
        return [[]]*12+["Veuillez uploader un fichier pour commencer.","primary","secondary","secondary","secondary",active_view]

    required_cols={"Réf. Réclamation","AGENCE","Annee","Mois","SEGMENTATION","DELAI_RECLAMATION"}
    if df.empty or not required_cols.issubset(df.columns):
        return [[]]*12+["⚠️ Données invalides. Ré-uploadez le fichier.","primary","secondary","secondary","secondary",active_view]

    options_ticket=[{'label':str(x),'value':x} for x in sorted(filter_df(df,filters,"Réf. Réclamation")['Réf. Réclamation'].dropna().unique())]
